...
class Tagger:
    def __init__(self):
        ...
        #self.button_savetags = Button(self.master, text="Save Tags", command=self.SaveTags, bg='#9657C7', fg='white', height=2, width=10, font=("Verdana", 8, "bold"))
    ...
    def ShowSentences(self):
        ...
        #self.button_savetags.grid(row=2,column=1, padx=5, pady=5)
    ...
    def PER_NE(self):
        try:
            ...
            if "per" in self.sentence_text.tag_names('sel.first'):
                ...
                self.persons.append(selected)
            print(selected)
            self.statustags.config(text="PER: {}, ORG: {}, LOC: {}".format(self.persons,self.organizations,self.locations))
        except:
            pass
    ...
    def SaveTags(self):
        ...
        self.per_entities.insert(self.current, self.persons)
        self.org_entities.insert(self.current, self.organizations)
