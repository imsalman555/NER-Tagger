...
class Tagger:
    ...
    def SaveTags(self):
        ...
        if self.per_entities[self.current] != '':
            self.per_count[self.current] = len(self.per_entities[self.current])
        else:
            self.per_count[self.current] = 0
        self.org_entities[self.current] = ','.join(self.organizations)
        if self.org_entities[self.current] != '':
            self.org_count[self.current] = len(self.org_entities[self.current])
        else:
            self.org_count[self.current] = 0
        self.loc_entities[self.current] = ','.join(self.locations)
        if self.loc_entities[self.current] != '':
            self.loc_count[self.current] = len(self.loc_entities[self.current])
        else:
    ...
    def SaveFile(self):
        ...
        tagged_data = pd.DataFrame({"Sentences":self.sentences, "PER":self.per_entities, "ORG":self.org_entities, "LOC":self.loc_entities, "PER_COUNT":self.per_count, "ORG_COUNT":self.org_count, "LOC_COUNT":self.loc_count})
        try:
